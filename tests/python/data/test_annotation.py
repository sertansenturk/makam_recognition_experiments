...
class TestAnnotation:
    ...
    def test_from_mlflow(self):
        ...
        # WHEN; THEN
        with mock.patch("mre.data.annotation.get_run_by_name", return_value=mock_run):
            with mock.patch(
                ...
                with mock.patch.object(
                    ...
                    with mock.patch(
                        "pandas.read_json", autospec=True
                    ) as mock_read_json:
                        _ = Annotation.from_mlflow()
    ...
    def test_validate_num_recordings(self):
        # GIVEN
        annotation = Annotation()
        annotation.data = pd.DataFrame([{"col1": "val1"}, {"col1": "val2"}])

        # WHEN; THEN
        with mock.patch.object(annotation, "EXPECTED_NUM_RECORDINGS", 2):
    ...
    def test_validate_unexpected_num_recordings(self):
        ...
        # WHEN; THEN
        with mock.patch.object(annotation, "EXPECTED_NUM_RECORDINGS", 3):  # not 2
            with pytest.raises(ValueError):
                annotation._validate_num_recordings()
    ...
    def test_validate_mbids(self):
        # GIVEN
        annotation = Annotation()
        annotation.data = pd.DataFrame([{"mbid": "id1"}, {"mbid": "id2"}])
    ...
    def test_validate_nonunique_mbids(self):
        # GIVEN
        annotation = Annotation()
        annotation.data = pd.DataFrame([{"mbid": "id1"}, {"mbid": "id1"}])
    ...
    def test_val_num_recs_per_makam(self, data, expected):
        ...
        # WHEN; THEN
        with mock.patch.object(
            annotation, "EXPECTED_NUM_RECORDINGS_PER_MAKAM", expected
        ):
    ...
    def test_val_num_recs_per_makam_unexpected(self):
        ...
        annotation = Annotation()
        annotation.data = pd.DataFrame(
            [{"makam": "makam1"}, {"makam": "makam2"}]  # 2 recordings per makams
        )

        # WHEN; THEN
        with mock.patch.object(
            annotation, "EXPECTED_NUM_RECORDINGS_PER_MAKAM", 5
        ):  # not 2
            with pytest.raises(ValueError):
                annotation._validate_num_recordings_per_makam()
    ...
    def test_validate_num_makams(self, data, expected):
        ...
        # WHEN; THEN
        with mock.patch.object(annotation, "EXPECTED_NUM_MAKAMS", expected):
    ...
    def test_validate_num_makams_incorrect(self):
        ...
        annotation = Annotation()
        annotation.data = pd.DataFrame(
            [{"makam": "makam1"}, {"makam": "makam2"}, {"makam": "makam3"}]  # 3 makams
        )

        # WHEN; THEN
        with mock.patch.object(annotation, "EXPECTED_NUM_MAKAMS", 5):  # not 3
            with pytest.raises(ValueError):
                annotation._validate_num_makams()

    @mock.patch("mre.data.Annotation._parse_mbid_urls")
    @mock.patch("mre.data.Annotation._patch_dunya_uids")
    def test_transform(self, mock_patch_dunya_uids, mock_parse_mbid_urls, mock_tmp_dir):
        # GIVEN
        annotation = Annotation()
        annotation.data = pd.DataFrame([{"col1": "val1"}, {"col1": "val2"}])
        ...
        with mock.patch(
            ...
            with mock.patch.object(annotation.data, "to_json") as mock_to_json:
    ...
    @mock.patch("mre.data.Annotation._parse_mbid_urls")
    @mock.patch("mre.data.Annotation._patch_dunya_uids")
    def test_transform_existing_tmp_dir(
        self, mock_patch_dunya_uids, mock_parse_mbid_urls, mock_tmp_dir
    ):
        # GIVEN
        annotation = Annotation()
        annotation.data = pd.DataFrame([{"col1": "val1"}, {"col1": "val2"}])
        ...
        # WHEN
        with mock.patch.object(annotation, "_cleanup", autospec=True) as mock_cleanup:
            with mock.patch(
                "tempfile.TemporaryDirectory", autospec=True, return_value=mock_tmp_dir
            ):
                with mock.patch.object(annotation.data, "to_json") as mock_to_json:
